...
int submission(State *state, int depth, int alpha, int beta, int maximizing)
{
  ...
  if(maximizing)
  {
    ...
    for(auto it = state->legal_actions.begin(); it != state->legal_actions.end(); it++)
    {
      ...
      int val = submission(temp, depth-1, alpha, beta, 0);
      if(val > value)
      {
        value = val;
      }
  ...
  else
  {
    ...
    for(auto it = state->legal_actions.begin(); it != state->legal_actions.end(); it++)
    {
      ...
      int val = submission(temp, depth-1, alpha, beta, 1);
      if(val < value)
      {
        value = val;
      }
...
{  
  submission(state, depth, -1000000000, 1000000000, 1-state->player);
  return state->ret;
}